const { parseEvent, transformRecipients, fetchMessage, handler, deleteMessage, processMessage } = require('./code/email-forward');
...
describe('email-forward', () => {
    ...
    describe('handler', () => {
        ...
        it('should handle multiple recipients', async () => {
            ...
            const testData = { ...data };
            testData.event = { ...testData.event, Records: [{ ...testData.event.Records[0], ses: { ...testData.event.Records[0].ses, receipt: { ...testData.event.Records[0].ses.receipt, recipients: ['test@thing.com', 'test2@thing.com'] } } }] };
            testData.emailData = `From: sender@example.com\r\nTo: test@thing.com,test2@thing.com\r\nSubject: Lorem Ipsum Test Email\r\n\r\n` + emailBody;
            const s3 = {
                ...testData.s3,
                getObject: jest.fn().mockImplementation((params, callback) => {
                    callback(null, { Body: testData.emailData });
                    return { promise: () => Promise.resolve({ Body: testData.emailData }) };
                }),
            };
            ...
            expect(testData.ses.sendRawEmail).toHaveBeenCalledWith(
                {
                    Destinations: ['forward@zz.com', 'forward2@zz.com'],
                    Source: 'test@thing.com',
                    RawMessage: { Data: `From: "sender@example.com via thing.com" <test@thing.com>\r\nTo: forward@zz.com,forward2@zz.com\r\nSubject: Lorem Ipsum Test Email\r\nReply-To: "Original Sender" sender@example.com\r\nList-Id: Forwarded emails via thing.com <bounce.thing.com>\r\nX-Forwarded-For: sender@example.com\r\n\r\n` + emailBody }
                },
                expect.any(Function),
            );
            expect(testData.ses.sendRawEmail).toHaveBeenCalledWith(
                {
                    Destinations: ['forward@zz.com', 'forward2@zz.com', 'forward3@zz.com'],
                    Source: 'test2@thing.com',
                    RawMessage: { Data: `From: "sender@example.com via thing.com" <test2@thing.com>\r\nTo: forward@zz.com,forward2@zz.com,forward3@zz.com\r\nSubject: Lorem Ipsum Test Email\r\nReply-To: "Original Sender" sender@example.com\r\nList-Id: Forwarded emails via thing.com <bounce.thing.com>\r\nX-Forwarded-For: sender@example.com\r\n\r\n` + emailBody }
                },
                expect.any(Function),
            );
    ...
    describe('processMessage', () => {
        it('should handle CC recipients correctly', async () => {
            const testData = { ...data };
            testData.event = { ...testData.event, Records: [{ ...testData.event.Records[0], ses: { ...testData.event.Records[0].ses, receipt: { ...testData.event.Records[0].ses.receipt, recipients: ['test@thing.com', 'test2@thing.com'], commonHeaders: { cc: ['cc1@example.com', 'cc2@example.com'] } } } }] };
    ...
    describe('transformRecipients', () => {
        ...
        it('should transform recipients', async () => {
            ...
            expect(result.recipients).toEqual({ 'test@thing.com': ['forward@zz.com', 'forward2@zz.com'] });
        });
        ...
        it('should transform multiple recipients', async () => {
            ...
            const parsedData = await parseEvent(testData)
            const result = await transformRecipients(parsedData);
            expect(result.recipients).toEqual({ 'test@thing.com': ['forward@zz.com', 'forward2@zz.com'], 'test2@thing.com': ['forward@zz.com', 'forward2@zz.com', 'forward3@zz.com'] });
        });
    });
